...
from (SELECT  i.Status as Injury
	  ...
	  (select CrashID,MostSevereInjury from Crashes
	  where CountOfNonMotorists != 0) as c
	  on c.CrashID = p.CrashID

	  inner join

	  (select Value,Status from injurystatus) as i
	  on i.Value = p.InjuryStatus

	  where p.Age is not Null and MostSevereInjury is not null and p.InjuryStatus is not null and AgeRange is not null
	  group by i.Status,a.AgeRange
